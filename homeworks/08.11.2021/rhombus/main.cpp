...
void rhombus(int n, int m)                                                  //функция выводящая ромбики
{
    for ( int k = 1; k <= (2 * n - 1); k++ )                                //перемещаемся от первой до 2*n-1 строки
    {
        ...
        {
            for (int s = 0; s <  abs ((n - k) % n); s++ )                   //выводит количество пробелов слева от звёзд
                std::cout << ' ';
            for (int d = 0; d < ( ( n - 1 ) - abs( (n - k) % n ) ); d++ )   //выводит звёзды слева от центральной звезды
                std::cout << '*';
            std::cout << '*';                                               //выводится центральная звезда
            for (int d = 0; d < ( ( n - 1 ) - abs( (n - k) % n ) ); d++ )   // выводит звёзды справа от центральной звезды
                std::cout << '*';
            for (int s = 0; s < abs( (n - k) % n) ; s++ )                   //выводит количество пробелов справа от звезд
                std::cout << ' ';
